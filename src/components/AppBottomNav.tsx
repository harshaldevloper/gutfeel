...
const tabs = [
  ...
  { href: "/tracker", label: "Track", icon: "tracker" },
...
function TabIcon({ icon, active }: { icon: (typeof tabs)[number]["icon"]; active: boolean }) {
  const cls = `w-5 h-5 ${active ? "text-brand-green" : "text-stone-400"}`;
  switch (icon) {
    case "home":
      return (
        <svg className={cls} fill={active ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24" strokeWidth={active ? 0 : 1.75}>
          {active ? (
